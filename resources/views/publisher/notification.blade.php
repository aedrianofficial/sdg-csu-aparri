                            ...
                            <div class="list-group">
                                @forelse (auth()->user()->notifications()->latest()->get() as $notification)
                                    @php
                                        ...
                                        // Determine appropriate route based on type and status for publishers
                                        $route = '';
                                        if ($data['type'] === 'project') {
                                            $route = match ($data['status']) {
                                                'submitted for publishing' => route('publisher.projects.show', [
                                                    'id' => $notification->related_id,
                                                    'notification_id' => $notification->id,
                                                ]),
                                                default => route('publisher.projects.show', [
                                                    'id' => $notification->related_id,
                                                    'notification_id' => $notification->id,
                                                ]),
                                            };
                                            ...
                                            $route = match ($data['status']) {
                                                ...
                                                default => route('publisher.research.show', [
                                                    'id' => $notification->related_id,
                                                    'notification_id' => $notification->id,
                                                ]),
                                            };
