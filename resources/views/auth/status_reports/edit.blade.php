                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form method="post" action="{{ route('auth.status_reports.update', $statusReport->id) }}" class="needs-validation"
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control" id="log_status" name="log_status" required>
                                        <option value="Proposed"
                                            {{ old('log_status', $statusReport->log_status) == "Proposed" ? 'selected' : '' }}>
                                            Proposed</option>
                                        <option value="On-Going"
                                            {{ old('log_status', $statusReport->log_status) == "On-Going" ? 'selected' : '' }}>
                                            On-Going</option>
                                        <option value="On-Hold"
                                            {{ old('log_status', $statusReport->log_status) == "On-Hold" ? 'selected' : '' }}>
                                            On-Hold</option>
                                        <option value="Rejected"
                                            {{ old('log_status', $statusReport->log_status) == "Rejected" ? 'selected' : '' }}>
                                            Rejected</option>
                                    </select>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="review_status_id" id="review_status_id" class="form-select" required>
                                        @foreach ($reviewStatuses as $status)
                                            <option value="{{ $status->id }}" @selected(old('review_status_id', $statusReport->review_status_id) == $status->id)>
                                                {{ $status->status }}
                                            </option>
                                        @endforeach
                                    </select>
            ...
            if (isDirty) {
                ...
                if (confirm(confirmationMessage)) {
                    ...
                    window.location.href =
                        '{{ route('auth.status_reports.index') }}'; // Redirect to home or desired route
                }
            } else {
                window.location.href =
                    '{{ route('auth.status_reports.index') }}'; // Redirect to home or desired route
