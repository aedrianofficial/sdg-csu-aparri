                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form action="{{ route('approver.status_reports.approved_list') }}" method="GET"
                                class="mb-4">
                                <div class="row">
                                    <!-- Title Filter -->
                            ...
                            <div class="table-responsive">
                                @if (count($statusReportsPaginated) > 0)
                                    <table id="reports-table" class="table table-striped">
                                        ...
                                        <tbody>
                                            @foreach ($statusReportsPaginated as $report)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if ($report->related_type === App\Models\Project::class)
                                                            <a href="{{ route('approver.status_reports.show_approved_project', $report->id) }}"
                                                                class="btn btn-sm btn-success">View</a>
                                                        @elseif ($report->related_type === App\Models\Research::class)
                                                            <a href="{{ route('approver.status_reports.show_approved_research', $report->id) }}"
                                                                class="btn btn-sm btn-success">View</a>
                                                        @endif
                                                    </td>
                                ...
                                @if (count($statusReportsPaginated) > 0)
                                    <nav aria-label="Page navigation">
                                        <ul class="pagination justify-content-center">
                                            <!-- Previous Button -->
                                            <li class="page-item {{ $statusReportsPaginated->onFirstPage() ? 'disabled' : '' }}">
                                                <a class="page-link"
                                                    href="{{ $statusReportsPaginated->previousPageUrl() . '&' . http_build_query(request()->except('page')) }}"
                                                    tabindex="-1">Previous</a>
                                            </li>

                                            <!-- Page Number Links -->
                                            @php
                                                $currentPage = $statusReportsPaginated->currentPage();
                                                $lastPage = $statusReportsPaginated->lastPage();
                                            ...
                                            @for ($i = $start; $i <= $end; $i++)
                                                <li class="page-item {{ $currentPage == $i ? 'active' : '' }}">
                                                    <a class="page-link"
                                                        href="{{ $statusReportsPaginated->url($i) . '&' . http_build_query(request()->except('page')) }}">{{ $i }}</a>
                                                </li>
                                            @endfor

                                            <!-- Next Button -->
                                            <li class="page-item {{ $statusReportsPaginated->hasMorePages() ? '' : 'disabled' }}">
                                                <a class="page-link"
                                                    href="{{ $statusReportsPaginated->nextPageUrl() . '&' . http_build_query(request()->except('page')) }}">Next</a>
                                            </li>
                                        </ul>
                                    </nav>
                                @endif
